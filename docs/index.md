# sandboxctl

A command-line tool for managing isolated development sandboxes powered by [NVIDIA OpenShell](https://github.com/NVIDIA/openshell).

!!! warning "Experimental"

    This project is experimental. APIs and configuration formats may change.

## Overview

sandboxctl wraps the OpenShell CLI to provide profile-based configuration, credential management, and lifecycle commands for GPU-accelerated development containers.

## Documentation

- **[Getting Started](getting-started.md)** -- Installation, prerequisites, and your first sandbox.
- **[Commands](commands/index.md)** -- Reference for every CLI command.
- **[Configuration](configuration/index.md)** -- Profiles, credentials, and settings.
- **[Contributing](contributing.md)** -- How to contribute to the project.
